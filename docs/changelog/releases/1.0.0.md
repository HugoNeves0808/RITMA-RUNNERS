...
This file keeps the implementation history and release notes that led to the first major project release and the completion of phase one.

## Release Summary

### First major project release and completion of phase one
...
- Reorganized written project documentation under `docs/`, split the API documentation into domain-specific files for easier navigation, and kept simple root-level redirect documents for compatibility.
- Removed the old Excel-based manual test checklist from the repository as part of the documentation cleanup around the finalized `1.0.0` release.
...
- Seeded temporary default accounts through Flyway instead of hardcoding runtime admin creation.
