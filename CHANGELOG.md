...
- Refined the web `Races` card layout again so status-driven rows now use a denser compact presentation for `Cancelled`, `Not registered`, `Did not start`, and `Did not finish`, while `Registered` cards now surface `circuit` and scheduled start `time` directly in the visible metrics.
- Extended the backend `/api/races/table` payload with `circuitId` and `circuitName`, allowing the web list to show registered-race circuit context without an extra details fetch.
- Corrected the web add-race refresh flow so the drawer closes only after the parent refresh finishes, preventing the newly created race from disappearing behind stale list state.
- Simplified the web `Best Efforts` category counters by removing the redundant clickable `total` drill-down and keeping only the meaningful `valid` and `excluded` race-list entry points.
- Softened empty-state borders on the web `Best Efforts` podium and category cards to match the newer product surface treatment.
- Adjusted web login validation so the `Email` field waits for blur before showing validation feedback, reducing noise while the user is still typing.
