...
- web `Races` also includes a real card-based list mode grouped by year, with a sticky right-side filters panel, collapsible checkbox-based filters, independent collapse controls for each year, a `Coming Up` section for registered races, and a top `In List` section that combines dated and undated in-list races into a lighter two-line list while hiding undated entries whenever a specific year filter is selected
...
- web `Races` creation and edit now require `race type` for every status, mark that selector as required in the form, and let the managed `race type`, `team`, `circuit`, and `shoe` selectors be searched directly inside the dropdowns
